 ...
 */
#ifndef BICYCL_ARITH_GMP_EXTRAS_INL
#define BICYCL_ARITH_GMP_EXTRAS_INL

#include "gmp_extras.hpp"
...
namespace BICYCL
{
  ...
  {
    /* the binary data is interpreted most significant byte first */
  ...
  /* */
  inline
  Mpz::operator std::vector<unsigned char> () const
  {
    /* absolute value exported as binary data, most significant byte first */
    size_t nbytes = (this->nbits() + 7u) / 8u;
    std::vector<unsigned char> vec(nbytes);
    mpz_export(vec.data(), NULL, 1, 1, 0, 0, static_cast<mpz_srcptr>(mpz_));
    return vec;
  }
  ...
  */
  ...
  /* */
  inline
  void Mpz::exor (Mpz &r, const Mpz &op1, const Mpz &op2)
  {
    mpz_xor (r.mpz_, op1.mpz_, op2.mpz_);
  }
  ...
  */
  ...
  /* */
  inline
  void Mpz::factorial(Mpz &r, unsigned long int n)
  {
    mpz_fac_ui(r.mpz_, n);
  }
  ...
  */
  ...
  /* */
  WNAF::WNAF(const Mpz & n, unsigned int w)
  {
    // Min w is 2 (1-NAF does not make sense)
    // Max supported w is 16 (min/max values of 16-NAF digits is -+(2^15 -1))
    if ((w < 2u) || (w > 16u))
      std::invalid_argument("WNAF: w must be in [2, 16]");

    // WNAF size is at most 1 more than the bitsize of n
    reserve(n.nbits() +1);

    Mpz aux;
    Mpz n_copy{n};
    while (n_copy.sgn() != 0) {
      if (n_copy.tstbit (0)) {   // k%2
        Mpz::mod2k_centered(aux, n_copy, w);  // aux = k mod_c 2^w
        push_back(static_cast<int16_t>(static_cast<long>(aux)));
        Mpz::sub(n_copy, n_copy, aux);   // k = k - aux
      }
      else {
        push_back(0);
      }
      Mpz::divby2(n_copy, n_copy);  // k = k//2
    }
  }

  /* */
  int16_t WNAF::operator[] (size_t i) const
  {
    if (i < size())
      return std::vector<int16_t>::operator[] (i);
    else
      return 0;
  }

  size_t nbits(mp_limb_t n)
  {
    return mpn_sizeinbase(&n, 1, 2);
  }
...
#endif /* BICYCL_ARITH_GMP_EXTRAS_INL */
