 ...
 */
#ifndef BICYCL_ARITH_OPENSSL_WRAPPER_INL
#define BICYCL_ARITH_OPENSSL_WRAPPER_INL
...
namespace BICYCL {
  ...
  /* */
  inline
  HashAlgo::HashAlgo (int nid, size_t digest_nbytes) : md_(EVP_get_digestbynid (nid)),
                                mdctx_ (new_ctx_())
  {
    if (md_ == NULL)
    {
    ...
    if (nid == NID_shake128 || nid == NID_shake256)
    {
      if (digest_nbytes == 0u)
      {
        EVP_MD_CTX_free(mdctx_);
        throw std::invalid_argument ("digest byte length cannot be zero");
      }
      digest_nbytes_ = digest_nbytes;
    }
    else
    {
      digest_nbytes_ = EVP_MD_size(md_);
    }
  }
  ...
  /* */
  inline HashAlgo::HashAlgo(const HashAlgo & H)
    : md_(H.md_),
      mdctx_(new_ctx_()),
      digest_nbytes_(H.digest_nbytes_)
  {
    try
    {
      operator= (H);
  ...
  /* */
  inline HashAlgo::HashAlgo(HashAlgo && H)
    : md_(H.md_),
      mdctx_(H.mdctx_),
      digest_nbytes_(H.digest_nbytes_)
  {
  ...
  HashAlgo::Digest HashAlgo::operator() (const Args & ...args)
  {
      ...
      std::unique_lock<std::mutex> lock{mutex_}; // Lock mutex, exception-safe
    #endif
    ...
    Digest h (digest_nbytes_);
    if (digest_nbytes_ !=  (size_t)EVP_MD_size (md_))
      // Extendable output function with custom digest length
      ret = EVP_DigestFinalXOF (mdctx_, h.data(), digest_nbytes_);
    else
  ...
  /* */
  inline
  BN::operator std::vector<unsigned char> () const
  {
    return BN::BIGNUM_abs_to_bytes (bn_);
  }
  ...
  /* */
  inline
  ECPoint::ECPoint (const ECGroup &E, const BN & x, const BN & y)
    : ECPoint (E)
  {
    E.set_coords_of_point(*this, x, y);
  }
  ...
  */
  ...
  inline
  void ECGroup::set_coords_of_point (ECPoint &P, const BN &x, const BN &y) const
  {
    int ret
        = EC_POINT_set_affine_coordinates(ec_group_, P.P_, x.bn_, y.bn_, ctx_);
    if (ret != 1)
      throw std::runtime_error ("Could not set coordinates");
  }
  ...
  /* */
  inline
  void ECGroup::neg_mod_order (BN &r, const BN &a) const
  {
    int ret = BN_mod_sub (r.bn_, get_order(), a.bn_, get_order (), ctx_);
    if (ret != 1)
      throw std::runtime_error ("BN_mod_sub failed");
  }
  ...
  /* */
  inline
  BN ECGroup::random_mod_order (RandGen & randgen) const
  {
    Mpz r{randgen.random_mpz(order_)};
    return static_cast<BN>(r);
  }
...
#endif /* BICYCL_ARITH_OPENSSL_WRAPPER_INL */
