 ...
 */
#ifndef BICYCL_ARITH_QFI_INL
#define BICYCL_ARITH_QFI_INL
...
/* */
  inline void QFI::nupow(QFI & r,
                         const QFI & f,
                         const Mpz & n,
                         const Mpz & L)
  {
    size_t nbits = n.nbits();
    unsigned int w;

    /*
     * Coarse tuning of w for wNAF*, relative to the bits of the exponent.
     * The values were choosen after several executions of the nupow benchmark
     * (benchs/benchmark_qfi.cpp in the repository).
     * These values provide good performance in general.
     * An optimal w could be computed from the analysis of the number of operations
     * for each w, but it would likely not give a significant performance increase.
     */
    if (nbits < 30u)        /*         n < 30 bits    w = 3 */
      w = 3u;
    else if (nbits < 100u)  /*   30 <= n < 100 bits   w = 4 */
      w = 4u;
    else if (nbits < 300u)  /*  100 <= n < 300 bits   w = 5 */
      w = 5u;
    else if (nbits < 700u)  /*  500 <= n < 700 bits   w = 6 */
      w = 6u;
    else if (nbits < 3000u) /* 1000 <= n < 3000 bits  w = 7 */
      w = 7u;
    else                    /* 3000 <= n bits         w = 8 */
      w = 8u;
  ...
  *  - w: window size for windowed NAF method. Requires w > 1. Default w = 7
  * Ouput:s
  ...
  */
  ...
  /**
  * \brief Exponentiation with precomputations
  *
  * \param[out] r a qfi corresponding to f^n
  * \param[in] f a qfi, the base for exponentiation
  * \param[in] n a positive integer, the exponent
  * \param[in] d an even positive integer
  * \param[in] fe a qfi such that fe = f^(2^(d/2))
  * \param[in] fd a qfi such that fd = f^(2^d)
  * \param[in] fed a qfi such that fed = f^(2^(3*d/2)) = f^(2^(d+d/2))
  * \param[in] L the bound for partial reduction in nucomp and nudupl
  *
  * \pre
  *   n is a positive integer
  *   d is an even positive integer
  *   fe, fd, fed are precomputed such that :
  *     - fe = f^(2^(d/2))
  *     - fd = f^(2^d)
  *     - fed = f^(2^(3*d/2)) = f^(2^(d+d/2)))
  *
  * Implementation:
  *    A fixed-base comb method is used to run through a joint sparse form (JSF)
  *    of the exponent.
  *    n is written n = extra*2^(2*d) + n_high*2^d + n_low
  *        +-------+-------------------------+-------------------------+
  *        | extra |          n_high         |          n_low          |
  *        +-------+-------------------------+-------------------------+
  *               2*d                        d                         0
  *
  *    A JSF of (n_high, n_low) is computed; its length is d or d+1.
  *    TODO Add ref to paper where JSF is detailed
  *
  *    Joint Sparse Form (n_high, n_low):
  *        +-----------------------------------+
  *        |             n_high_JSF            |
  *        +-----------------------------------+
  *        +-----------------------------------+
  ...
  *       d+1                e                 0
  *
  *    If the JSF has length d+1, the additional digit is handled separately.
  *    Then, if extra is not 0, fed^(extra) is computed and composed into r
  *
  *    Lastly the d remaining digits of the JSF are parsed using a comb of size 2
  *    (so e:=d/2 iterations are performed, with one nudupl per iteration)
  *
  *        +--+------|------------+------|-----------+
  *        |  |      |            |      |           |  n_high_JSF
  *        +--+------|------------+------|-----------+
  ...
  *            e = d/2 iterations
  *
  *    Precomputed values are composed into r with the square-and-multiply method
  *
  *    This exponentition algorithm is most efficient when n has 2*d bits
  */
  inline
  void QFI::nupow(QFI & r,
                  const QFI & f,
                  const Mpz & n,
                  const QFIPrecomp & pre,
                  const Mpz & L)
  {
    if (n.is_zero ())
    {
    ...
    else if (n.nbits() < pre.e())
    {
      nupow (r, f, n, L);
    }
    else
    {
      Mpz n_low, n_high, extra;
      QFI::OpsAuxVars tmp;

      Mpz::mod2k (n_low, n, pre.d());         // n_low <-- keep bits d-1 to 0
      Mpz::divby2k (n_high, n, pre.d());      // n_high <-- keep bits above d

      // If number of bits of n > 2*d then
      //   - Chop n_high to d bits
      //   - Store the extra bits (start from the 2d-th bit)
      if (n.nbits() > 2*pre.d())
      {
        Mpz::divby2k (extra, n_high, pre.d());    // extra <-- keep bits above 2d-1
        Mpz::mod2k (n_high, n_high, pre.d());     // n_high <-- keep bits 2d-1 to d
      }

      /* */
      JSF jsf (n_low, n_high);
      ...
      */
      ...
      // Handle the possible additional digit of JSF representation
      if (jsf.size() > 2*pre.e())
      {
        uint8_t digh = jsf[2*pre.e()];
      ...
      // To handle the extra bits (part of n starting from the 2d-th bit),
      // compose with r = fed^extra = f^(extra*2^(e+d))
      // In the next loop, r is squared exactly e times.
      // Thus we get r^(2^e) = f^(extra*2^(2e+d)) = f^(extra*2^(2d))
      if (!extra.is_zero())
      {
        QFI f_extra;
        nupow(f_extra, pre.gde(), extra, L, 5u);
        nucomp (r, r, f_extra, L, false, tmp);
      }

      // Parse the JSF using a comb algorithm of size 2.
      // When needed, compose with precomputed values
      for (size_t j = 2*pre.e(); j > pre.e(); j--)
      {
        uint8_t digh = jsf[j-1];
        uint8_t digl = jsf[j-pre.e()-1];
  ...
  inline
  void QFI::nupow(QFI & r,
                  const std::vector<QFI> & fi,
                  const std::vector<Mpz> & ni,
                  unsigned int w,
                  const Mpz & L)
  {
    // cf. Algo 3.51 Interleaving with NAFs, Guide to ECC Hankerson, Menezes, Vanstone

    // 1 - check vectors sizes (must be the same for fi and ni, call it m)
    size_t m = fi.size();
    if ((m == 0u) || (m != ni.size()))
      std::invalid_argument("Vectors must have the same non-null size");

    // 2 - Compute wNAF forms for all ni. Store the bitsize for parsing
    size_t biggest_bitsize = 0;
    std::vector<WNAF> wnaf;
    wnaf.reserve(m);
    for (unsigned int i = 0; i < m; ++i) {
      wnaf.emplace_back(ni[i], w);
      if ( wnaf[i].size() > biggest_bitsize)
        biggest_bitsize = wnaf[i].size();
    }

    // 3 - Perform m * 2^(w-2) precomputations
    // precomps:
    //   [ f0, f0^3, ... ,     f0^(2^(w-1) - 1)   ]
    //   [           ...                          ]
    //   [ fm-1, fm-1^3, ... , fm-1^(2^(w-1) - 1) ]
    size_t nb_precomp_per_f = (1u << (w-2));
    std::vector<std::vector<QFI>> precomps;
    precomps.resize(m);

    BICYCL::QFI ff;
    for (unsigned int i = 0; i < m; ++i)
    {
      precomps[i].resize(nb_precomp_per_f);
      precomps[i][0] = fi[i];
      nudupl(ff, fi[i], L);   // fi^2
      for (unsigned int j = 1; j < nb_precomp_per_f; j++)
      {
        // fi^(2(j-1) + 1) * fi^2 =  fi^(2j+1)
        nucomp(precomps[i][j], precomps[i][j-1], ff, L, false);
      }
    }

    // 4 - Square and multiply
    // Parse all exponents left-to-right
    // For each exponentiation, multiply with the corresponding precomp value
    // Square jointly

    // Assumes all f have the same discriminant
    r = ClassGroup (fi[0].discriminant()).one();

    for (int i = biggest_bitsize ; i >= 0; --i) {
      // Squaring done jointly for all exponentiations
      nudupl(r, r, L);
      for (unsigned int j = 0; j < m; j++) {
        // Multiply with the corresponding precomp value
        if (wnaf[j][i] > 0)
          nucomp(r, r, precomps[j][(wnaf[j][i]-1)/2], L, false);
        if (wnaf[j][i] < 0)
          nucomp(r, r, precomps[j][(-wnaf[j][i]-1)/2], L, true);
      }
    }
  }
  ...
  /* */
  template <>
  inline void HashAlgo::hash(const QFI & f)
  {
    hash (f.a());
    hash (f.b());
    hash (f.c());
  ...
  template <>
  inline
  void HashAlgo::hash (const std::vector<QFI> &m)
  {
    for (const QFI & v : m)
      hash(v);
  }
  ...
  */
  ...
  inline void ClassGroup::nupow(QFI & r,
                                const QFI & f,
                                const Mpz & n) const
  {
  ...
  {
    QFI::nupow (r, f, n, default_nucomp_bound(), w);
  }
  ...
  inline void ClassGroup::nupow(QFI & r,
                                const QFI & f,
                                const Mpz & n,
                                const QFIPrecomp & pre) const
  {
    QFI::nupow (r, f, n, pre, default_nucomp_bound());
  }

  inline
  void ClassGroup::nupow(QFI & r,
                         const std::vector<QFI> & fi,
                         const std::vector<Mpz> & ni,
                         unsigned int w) const
  {
    QFI::nupow (r, fi, ni, w, default_nucomp_bound());
  }

  inline
  QFIPrecomp::QFIPrecomp(const QFI & g, size_t exp_bitsize)
  {
    ClassGroup group{g.discriminant()};
    ...
    for (size_t i = 0; i < d_+e_; i++)
    {
      ...
      if (i == d_)
        gd_ = gde_;
      group.nudupl (gde_, gde_);
    }
  }

  inline size_t QFIPrecomp::e() const
  {
    return e_;
  }

  inline size_t QFIPrecomp::d() const
  {
    return d_;
  }

  inline const QFI & QFIPrecomp::ge() const
  {
    return ge_;
  }

  inline const QFI & QFIPrecomp::gd() const
  {
    return gd_;
  }

  inline const QFI & QFIPrecomp::gde() const
  {
    return gde_;
  }
