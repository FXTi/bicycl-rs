 ...
 */
#ifndef BICYCL_CL_CL_HSMQK_INL
#define BICYCL_CL_CL_HSMQK_INL

#include "CL_HSMqk.hpp"
...
namespace BICYCL
{
  ...
  */
  ...
  CL_HSMqk::CL_HSMqk (const Mpz &q, size_t k, const Mpz &p,
      ...
      : q_(q),
        ...
        compact_variant_ (params.compact_variant),
        distance_ (params.distance)
  {
    ...
    if (params.distance < 2)
      throw std::invalid_argument ("distance must be >= 2");
    ...
    */
    ...
    if (params.compact_variant) /* For compact variant, we need \pi(h)^M */
    {
    ...
    * Precomputation
    */
    h_precomp_ = QFIPrecomp(h_, encrypt_randomness_bound().nbits());
  }
  ...
  */
  ...
  CL_HSMqk::CL_HSMqk (const Mpz &q, size_t k, size_t DeltaK_nbits,
                      RandGen &randgen, const CL_Params params)
    : CL_HSMqk (q, k, CL_HSMqk::random_p (randgen, q, DeltaK_nbits), params)
  {
  }
  ...
  */
  ...
  CL_HSMqk::CL_HSMqk (size_t q_nbits, size_t k, size_t DeltaK_nbits,
                      RandGen &randgen, const CL_Params params)
    : CL_HSMqk (randgen.random_prime(q_nbits), k, DeltaK_nbits, randgen, params)
  {
  }
  ...
  */
  ...
  CL_HSMqk::CL_HSMqk (size_t q_nbits, size_t k, SecLevel seclevel,
                      RandGen &randgen, const CL_Params params)
    : CL_HSMqk (randgen.random_prime(q_nbits), k, seclevel, randgen, params)
  {
    if (q_nbits < seclevel.nbits())
  ...
  */
  ...
  CL_HSMqk::CL_HSMqk (const Mpz &q, size_t k, SecLevel seclevel,
                      RandGen &randgen, const CL_Params params)
    : CL_HSMqk (q, k, seclevel.discriminant_bitsize(), randgen, params)
  {
    if (q.nbits() < seclevel.nbits())
      throw std::invalid_argument ("Number of bits of q should not be smaller "
                                  "than the security level");
  }
  ...
  /* */
  inline
  size_t CL_HSMqk::lambda_distance () const
  {
    return distance_;
  }
  ...
  */
  ...
  /* */
  inline CL_HSMqk::CipherText
  CL_HSMqk::addscal_ciphertexts(const PublicKey & pk,
                                const CipherText & ca,
                                const CipherText & cb,
                                const Mpz & s,
                                RandGen & randgen) const
  {
    Mpz r(randgen.random_mpz(encrypt_randomness_bound()));
    return addscal_ciphertexts(pk, ca, cb, s, r);
  }
  ...
  CL_HSMqk::addscal_ciphertexts(const PublicKey & pk,
                                ...
                                Mpz & r) const
  {
    return CipherText(*this, pk, ca, cb, s, r);
  }
  ...
  */
  ...
  template <>
  inline
  void HashAlgo::hash (const CL_HSMqk::PublicKey &k)
  {
    hash (k.elt());
  }
  ...
  template <>
  inline
  void HashAlgo::hash (const CL_HSMqk::CipherText &c)
  {
    hash (c.c1());
    hash (c.c2());
  }
  ...
  //                                                              const Mpz &t)
  //  : CL_HSMqk (cryptosystem), C_exp2_ (C_exp2), H_ (HashAlgo::SHAKE128)
  //{
  //}
  //
  ///* */
  ...
  /* */
  inline
  bool CL_HSMqk_ZKAoKProof::verify (const CL_HSMqk &C, HashAlgo &H,
                                    const CL_HSMqk::PublicKey &pk,
                                    const CL_HSMqk::CipherText &c) const
  {
    ...
    QFI cu1, cu2;
    C.power_of_h(cu1, u1_);
    pk.exponentiation(C, cu2, u1_);
    if (C.compact_variant())
      C.from_Cl_DeltaK_to_Cl_Delta (cu2);
    QFI temp{C.power_of_f(u2_)};
    C.Cl_Delta().nucomp(cu2, cu2, temp);


    /* ck = (c1^k, c2^k) */
    // CL_HSMqk::CipherText ck (C.scal_ciphertexts (pk, c, k_, Mpz (0UL)));
    // Rewrite the operations of CL_HSMqk::CipherText without the task parallelism.
    // It's not wanted in this case because the current function can be executed
    // inside other threads, and we do not want these threads to create sub-threads.
    QFI c1k, c2k;
    C.Cl_G().nupow(c1k, c.c1(), k_);
    C.Cl_Delta().nupow(c2k, c.c2(), k_);

    QFI t1, t2;
    /* Using the equality gq^u1 == t1*c1^k to compute t1 */
    C.Cl_G().nucompinv (t1, cu1, c1k);

    /* Using the equality pk^u1 f^u2 == t2*c2^k to compute t2 */
    C.Cl_Delta().nucompinv (t2, cu2, c2k);
  ...
  /* */
  inline
  Mpz CL_HSMqk_ZKAoKProof::k_from_hash (HashAlgo &H,
                                        const CL_HSMqk::PublicKey &pk,
                                        const CL_HSMqk::CipherText &c,
