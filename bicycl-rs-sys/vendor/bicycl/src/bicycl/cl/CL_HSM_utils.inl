 ...
 */
#ifndef BICYCL_CL_CL_HSM_UTILS_INL
#define BICYCL_CL_CL_HSM_UTILS_INL


#include "CL_HSM_utils.hpp"
...
namespace BICYCL { namespace _Utils
{
  ...
  {
    C.power_of_h (pk_, sk);
    pk_precomp_ = QFIPrecomp(pk_, C.encrypt_randomness_bound().nbits ());
  }
  ...
  : pk_{pk_QFI}
  {
    // Check pk_ hs the correct discriminant
    if (pk_.discriminant() != C.Cl_G().discriminant())
      throw std::invalid_argument(
          "pk's discriminant does not match Cryptosystem");
  ...
  {
    C.Cl_G().nupow (r, pk_, n, pk_precomp_);
  }
  ...
  {
    C.power_of_h (c1_, r); /* c1 = h^r */
    pk.exponentiation (C, c2_, r); /* c2 = pk^r */
    if (C.compact_variant())
      C.from_Cl_DeltaK_to_Cl_Delta (c2_);

    /* EvalScal(s, cb) without rerandomization */
    QFI tmp;
    C.Cl_G().nupow (tmp, cb.c1_, s);
    C.Cl_G().nucomp (c1_, c1_, tmp); /* c1 = h^r * cb1^s */
    C.Cl_Delta().nupow (tmp, cb.c2_, s);
    C.Cl_Delta().nucomp (c2_, c2_, tmp); /* c2 = pk^r * cb2^s */

    /* EvalAdd(ca, s.cb) */
    C.Cl_G().nucomp (c1_, c1_, ca.c1_); /* c1 = h^r * ca1 * cb1^s */
    C.Cl_Delta().nucomp (c2_, c2_, ca.c2_); /* c2 = pk^r * ca2 * cb2^s */
  }

  template <class Cryptosystem>
  inline CL_HSM_CipherText<Cryptosystem>::CL_HSM_CipherText(const QFI & c1,
                                                            const QFI & c2)
    : c1_{c1},
      c2_{c2}
  {
...
#endif /* BICYCL_CL_CL_HSM_UTILS_INL */
